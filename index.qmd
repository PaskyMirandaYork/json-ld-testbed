...
course_data.forEach(row => {
  ...
  // Use CSV column for type if it exists
  if (row.schemaVersion) jsonld["@type"] = row.schemaVersion;

  // @id and url
  if (row.identifier) jsonld["@id"] = row.identifier;
  if (row.url) jsonld["url"] = row.url;

  // Name and description
  if (row.headline) jsonld["name"] = row.headline;
  if (row.description) jsonld["description"] = row.description;

  // Educational fields
  if (row.learningResourceType) jsonld["educationalResourceType"] = row.learningResourceType;
  if (row.educationalLevel) jsonld["educationalLevel"] = row.educationalLevel;

  // Audience (comma-separated values)
  if (row.audience) {
    const audienceArray = row.audience.split(",").map(s => s.trim());
    if (audienceArray.length === 1) {
      jsonld["audience"] = { "@type": "Audience", "audienceType": audienceArray[0] };
    } else {
  ...
  if (row.projectFunding) jsonld["funding"] = row.projectFunding;
  if (row.sdPublisher) jsonld["publisher"] = { "@type": "Organization", "name": row.sdPublisher };
  ...
  if (row.inLanguage) jsonld["inLanguage"] = row.inLanguage;
  if (row.isAccessibleForFree != null) jsonld["isAccessibleForFree"] = row.isAccessibleForFree === "yes";

  // Optional metadata
  if (row.keywords) jsonld["keywords"] = row.keywords.split(",").map(k => k.trim());
  if (row.license) jsonld["license"] = row.license;
  if (row.author) jsonld["author"] = row.author;
  if (row.contributor) jsonld["contributor"] = row.contributor;
  if (row.maintainer) jsonld["maintainer"] = row.maintainer;
  if (row.version) jsonld["version"] = row.version;
