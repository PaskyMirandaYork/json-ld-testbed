...
if (!window.__courses_jsonld_added) {
  window.__courses_jsonld_added = true
  ...
  course_data.forEach(row => {
    const jsonld = {
      ...
      "@id": row.identifier || row.url,
      "url": row.url,
      "name": row.headline,
      "description": row.description,
      "learningResourceType": row.learningResourceType,
      "timeRequired": row.TimeRequired,
      "educationalLevel": row.audience,
      "keywords": [row.learningResourceType, row.audience].filter(Boolean),
      "isAccessibleForFree": true,
      "inLanguage": "en",
      "publisher": {
        "@type": "Organization",
        "name": "UKRI Digital Research Skills Catalyst"
      },
      ...(row.audience ? { audience: { "@type": "Audience", "audienceType": row.audience } } : {}),
      ...(row.projectFunding ? { provider: { "@type": "Organization", "name": row.projectFunding } } : {}),
      ...(row.teaches ? { teaches: row.teaches.split(",").map(t => t.trim()) } : {}),
      ...(row.competencyRequired ? { competencyRequired: row.competencyRequired.split(",").map(c => c.trim()) } : {})
    }
