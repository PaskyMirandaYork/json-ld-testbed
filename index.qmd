...
course_data.forEach(row => {
  ...
  // Build JSON-LD dynamically: include only fields present in the CSV
  const jsonld = { "@context": "https://schema.org" };

  // Add @type if it exists
  if (row.bioschemasType) jsonld["@type"] = row.bioschemasType;

  // Add @id or url if they exist
  if (row.url) jsonld["@id"] = row.url;
  ...
  if (row.audience) jsonld["audience"] = { "@type": "Audience", "audienceType": row.audience };
  if (row.projectFunding) jsonld["provider"] = { "@type": "Organization", "name": row.projectFunding };
  if (row.time_hours != null) jsonld["timeRequired"] = `PT${row.time_hours}H`;
  if (row.language) jsonld["inLanguage"] = row.language;
  if (row.isAccessibleForFree != null) jsonld["isAccessibleForFree"] = row.isAccessibleForFree;
  if (row.educationalLevel) jsonld["educationalLevel"] = row.educationalLevel;
  if (row.publisherName) jsonld["publisher"] = { "@type": "Organization", "name": row.publisherName };
