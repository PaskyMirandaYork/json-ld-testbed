...
course_data = pd.read_csv(
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vRSrvsfFfVwokza_WP9JIzd4Wfg6OKPBJcwelLTqYn1SgigZXnfcU6_apN5gWTMF79n4CRQFNOJ5w6M/pub?gid=1012757406&single=true&output=csv"
)
...
for _, row in course_data.iterrows():
    ...
    audience_list = [
        ...
        for a in str(row.get("audience","")).split(",") if a.strip()
    ]
    
    # Keywords (max 20, truncate each to 20 chars)
    keywords_list = [k.strip()[:20] for k in str(row.get("keywords","")).split(",")[:20] if k.strip()]

    # Compute timeRequired in hours
    time_required = "PT13H"  # default
    time_text = str(row.get("TimeRequired") or row.get("approxTimeRequired") or "")
    match = re.search(r"([\d\.]+)\s*(hour|day)", time_text, re.IGNORECASE)
    if match:
        hours = float(match.group(1))
        if match.group(2).lower().startswith("day"):
            hours *= 24
        time_required = f"PT{int(hours)}H"
    ...
    jsonld = {
        ...
        "@type": "Course",
        "@id": row["identifier"] if str(row.get("identifier","")).startswith("http") else row.get("url"),
        "url": row.get("url"),
        "name": row.get("headline"),
        "description": description,
        "learningResourceType": row.get("learningResourceType","Online Resource"),
        "educationalLevel": row.get("educationalLevel","level1"),
        "timeRequired": time_required,
        "inLanguage": row.get("inLanguage","en"),
        "isAccessibleForFree": str(row.get("isAccessibleForFree","yes")).lower() == "yes",
        "provider": {"@type": "Organization", "name": row.get("provider")},
        "audience": audience_list,
        "keywords": keywords_list,
        "hasCourseInstance": False
    }

    # Convert to <script> tag
    jsonld_scripts.append(f'<script type="application/ld+json">\n{json.dumps(jsonld, indent=2)}\n</script>')

# Expose to Quarto
ojs_define(jsonld_scripts=jsonld_scripts)
```
```{html}
