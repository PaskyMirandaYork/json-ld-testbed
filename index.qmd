...
// Prevent duplicates during reactive reruns
...
if (!window.__courses_jsonld_added) {
  ...
  course_data.forEach(row => {
    ...
    const jsonld = {
      ...
      "provider": row.projectFunding ? {
        "@type": "Organization",
        "name": row.projectFunding
      } : undefined,
      "timeRequired": row.TimeRequired,
      "teaches": row.teaches
        ? row.teaches.split(",").map(t => t.trim())
        : undefined,
      "competencyRequired": row.competencyRequired
        ? row.competencyRequired.split(",").map(t => t.trim())
        : undefined,
      "educationalLevel": row.audience,
      "keywords": [
        row.learningResourceType,
        row.audience
      ].filter(Boolean),
