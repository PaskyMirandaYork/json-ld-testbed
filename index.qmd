...
course_data.forEach(row => {
  ...
  // Keywords (trim to ~20 to avoid TeSS errors)
  if (row.keywords) {
    const keywordsArray = row.keywords.split(",").map(k => k.trim());
    jsonld["keywords"] = keywordsArray.slice(0, 20);
  }

  // Authors as Person objects
  if (row.author) {
    const authors = row.author.split(",").map(a => a.trim());
    jsonld["author"] = authors.map(a => ({ "@type": "Person", "name": a }));
  }
  ...
  if (row.inLanguage) jsonld["inLanguage"] = row.inLanguage.toLowerCase();
  jsonld["isAccessibleForFree"] = row.isAccessibleForFree === "yes";

  // Explicitly mark no course instance for TeSS
